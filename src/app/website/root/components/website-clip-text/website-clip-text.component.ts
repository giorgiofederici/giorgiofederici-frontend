...
export class WebsiteClipTextComponent implements OnInit, AfterViewInit, OnDestroy {

    @ViewChild('textContainer') textContainer: QueryList<ElementRef>;
    ...
    private hideTextLine() {
        ...
        this.animationPlayer = myAnimation.create(this.wordsWrapper.nativeElement, {});
    ...
    private showTextLine(textLine: ElementRef): void {
        const myAnimation = this.animationBuilder.build([
            style('*'),
            animate(this.animationOptions.revealDuration, style({ width: textLine.nativeElement.offsetWidth + 10 + 'px' }))
        ]);
    ...
    private switchTextLine(oldTextLine: ElementRef, newTextLine: ElementRef): void {
